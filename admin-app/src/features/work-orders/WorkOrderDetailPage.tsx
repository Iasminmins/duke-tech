...
import { priorityLabels } from '../../components/ui/status';
import { getWorkOrderDetail, updateWorkOrderStatus, getWhatsappTemplate, formatWorkOrderNumber, type WorkOrderDetail } from './workOrderRepository';
import { statuses, statusLabels, statusTone, priorityTone, type WorkOrderStatus } from './workOrderSchema';
import { buildWhatsappMessage, buildWhatsappUrl } from './whatsappMessage';
import { useAuth } from '../../auth/AuthProvider';
import { useToast } from '../../components/ui/ToastProvider';

const reminderStatuses: WorkOrderStatus[] = ['ready', 'delivered'];
...
export function WorkOrderDetailPage() {
  ...
  useEffect(() => { getWhatsappTemplate().then(({ whatsappMessage }) => setWhatsappTemplate(whatsappMessage)); }, []);
  ...
  const showReminder = reminderStatuses.includes(order.status);
  const reminderMessage = buildWhatsappMessage(whatsappTemplate, { cliente: customer?.full_name, aparelho: device ? `${device.brand} ${device.model}` : '', status: statusLabels[order.status], link: publicUrl, previsao: formatDate(order.estimated_due_date) });
  const reminderUrl = buildWhatsappUrl(customer?.phone || customer?.whatsapp, reminderMessage);
