...
export function WorkOrderDetailPage() {
  ...
  if (loading) return <section className="content"><div className="page-heading"><div><div className="eyebrow">Comanda</div><h2>Carregando comanda…</h2></div><Link className="btn" to="/admin/comandas">Voltar</Link></div><section className="panel empty">Carregando dados da comanda…</section></section>;
  if (error || !order) return <section className="content"><div className="page-heading"><div><div className="eyebrow">Comanda</div><h2>Comanda não encontrada</h2><p>{error || 'O registro solicitado não existe ou foi arquivado.'}</p></div><Link className="btn primary" to="/admin/comandas">Voltar para comandas</Link></div></section>;

  const currentOrder = order;
  async function saveStatus() { if (!id || !nextStatus || nextStatus === currentOrder.status) { setError('Escolha um status diferente do atual.'); return; } setSavingStatus(true); setError(''); const result = await updateWorkOrderStatus(id, nextStatus); if (result.error || !result.history) { setError('Não foi possível alterar o status.'); setSavingStatus(false); return; } setOrder({ ...currentOrder, status: nextStatus, updated_at: result.data?.updated_at || currentOrder.updated_at, work_order_status_history: [...currentOrder.work_order_status_history, result.history] }); setStatusOpen(false); setNextStatus(''); setSuccess('Status atualizado com sucesso.'); setSavingStatus(false); }
