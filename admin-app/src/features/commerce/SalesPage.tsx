  ...
  return <section className="content">
    ...
    <div className="dashboard-grid secondary-grid">
      ...
      <section className="panel">
        ...
        {!byMethod.length ? <div className="empty">Nenhuma venda registrada.</div> : methodView === 'bar' ? <div className="report-bars" style={{ marginTop: 16 }}>{byMethod.map(([method, value]) => <div className="report-bar-row" key={method}><span className="report-bar-label">{methods[method as keyof typeof methods] || 'Outro'}</span><div className="report-bar-track"><div className="report-bar-fill" style={{ width: `${(value / maxMethod) * 100}%` }} /></div><strong className="report-bar-value">{money(value)}</strong></div>)}</div> : <DonutChart data={byMethod.slice(0, 6).map(([method, value], index) => ({ key: method, label: methods[method as keyof typeof methods] || 'Outro', value, color: CATEGORY_PALETTE[index] }))} formatValue={money} />}
      </section>
    </div>
