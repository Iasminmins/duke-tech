...
type Device = { id: string; customer_id: string; brand: string; model: string; color: string | null; imei: string | null; serial_number: string | null; physical_condition: string | null; received_accessories: string | null; reported_problem: string; customers?: { full_name: string } | { full_name: string }[] | null; work_orders?: { id: string }[] };
...
export function DevicesPage() {
  ...
  const [rows, setRows] = useState<Device[]>([]); const [customers, setCustomers] = useState<Customer[]>([]); const [form, setForm] = useState<DeviceInput>(blank); const [open, setOpen] = useState(false); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [saving, setSaving] = useState(false);
  ...
  return <section className="content"><div className="page-toolbar-top"><span className="page-toolbar-count">{!loading && !error ? `${rows.length} ${rows.length === 1 ? 'aparelho' : 'aparelhos'}` : ''}</span><button className="btn primary" onClick={() => { setOpen(value => !value); setError(''); }}>+ Aparelho</button></div>
    ...
    {loading ? <div className="card loading-skeleton" role="status" aria-live="polite"><span /><span /><span /><span /></div> : !error && rows.length === 0 ? <section className="panel"><EmptyState icon="device" title="Nenhum aparelho cadastrado" description="Cadastre o aparelho para acompanhar o histórico de reparos do cliente." actionLabel="Cadastrar primeiro aparelho" actionTo="/admin/aparelhos" example="Depois você verá marca, modelo, cliente e problema relatado." /></section> : !error && <><div className="table-wrap"><table className="table device-table"><thead><tr><th>Aparelho</th><th>Cliente</th>{canSeeSensitiveInfo && <th>IMEI / série</th>}<th>Problema</th><th>Reparos</th></tr></thead><tbody>{pageItems.map(row => <tr key={row.id}><td><strong>{row.brand} {row.model}</strong>{row.color && <><br /><span className="muted">{row.color}</span></>}</td><td>{customerName(row.customers)}</td>{canSeeSensitiveInfo && <td className="muted">{row.imei || row.serial_number || '—'}</td>}<td className="device-problem-cell">{row.reported_problem}</td><td className="muted">{row.work_orders?.length ?? 0}</td></tr>)}</tbody></table></div><Pagination page={page} pageCount={pageCount} onChange={setPage} totalLabel={`${rows.length} ${rows.length === 1 ? 'aparelho' : 'aparelhos'}`} /></>}
  </section>;
}
