...
export function AgendaPage() {
  ...
  return <section className="content">
    <div className="page-heading"><div><div className="eyebrow">Operação</div><h2>Agenda</h2><p>Organize entregas, retornos e compromissos da assistência.</p></div><button className="btn primary" onClick={() => { setFormOpen(open => !open); setError(''); }}><Icon name="plus" size={16}/> {formOpen ? 'Fechar agendamento' : 'Agendar compromisso'}</button></div>
    ...
    <div className="module-overview agenda-overview">
      ...
      <article className="card module-stat"><span>Aguardando confirmação</span><strong>{loading ? '—' : metrics.awaitingConfirmation}</strong><small>{metrics.awaitingConfirmation ? 'Orçamentos aguardando retorno' : 'Tudo em dia'}</small></article>
    </div>
    <section className="panel agenda-panel">
      <header className="panel-header"><div><span className="section-kicker">Próximos compromissos</span><h3>Entregas e retiradas</h3></div><Link to="/admin/comandas" className="panel-link">Ver comandas <Icon name="arrow" size={14}/></Link></header>
